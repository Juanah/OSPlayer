...
namespace OSPlayerTest
{
	class MainClass
	{
		...
		public static void Main (string[] args)
		{
			...
			View baseView = GenerateTestView (ColorTranslator.FromHtml("#80deea"), 0.2f,-1);
			baseView.onPressed += (object sender) => {
				Console.WriteLine ("baseView has been clicked");
				((View)sender).offsetX += 0.03f;
			};
			View innerView = GenerateTestView (Color.Red, 0.01f,0);
		...
		private static View GenerateTestView(Color color,float size,int level)
		{
